  ...
  methods: {
        ...
        enterL(index,item){
            if(item.children){
                ...
                displayBox.forEach((item,i) => {
                    displayBox[index].style.backgroundColor = ""
                    displayBox[index].style.left = '100%';
                });
            }
        },
        enterT(index,item){
            if(item.children){
                let oli=document.getElementsByClassName('posi')[this.tabIndex];
                let navigationBox = oli.getElementsByTagName("p");
